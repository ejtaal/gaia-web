...
config_handler.set_global( length=120, spinner='classic', enrich_print = False, file = sys.stderr, force_tty = True)
...
def update_grand_db( grand_db, source_db):
    ...
    # if get_sqlite3_count_star( grand_db, 'grand_gaia_source', f'where source_id={df['source_id'].iloc[ 0]}')
    #     and get_sqlite3_count_star( grand_db, 'grand_gaia_source', f'where source_id={df['source_id'].iloc[ -1]}'):
    ...
    # added_to_db = 0
    try:
        save_db_stars.to_sql( 'grand_gaia_source', bigdb_conn, if_exists='append', index=False)
        row_count = get_sqlite3_count_star( grand_db, 'grand_gaia_source')
        grand_total_rows = row_count
        hm( f'Added {source_db}, grand_total_rows is now: {grand_total_rows:,}', '+')
        # exit(4)
...
def main():
    ...
    for sqlite_db in files_bar:
        files_bar.text(f' count so far: {total_rows_in_sources:,}') 
        # print( sqlite_db)
    ...
    grand_total_rows = 0
    try:
        hm( f'Counting rows in {GRAND_DB_FULLPATH} table grand_gaia_source ...')
        row_count = get_sqlite3_count_star( GRAND_DB_FULLPATH, 'grand_gaia_source')
        grand_total_rows = row_count
    except:
        exit(1)
        pass
    ...
    if total_rows_in_sources == grand_total_rows:
        hm( f"Total source rows match grand DB: {total_rows_in_sources:,}", '+')
    else:
        hm( f"Total rows in source DBs doesn't match grand DB total: {total_rows_in_sources:,} != {grand_total_rows:,}", '!')
    ...
    for dataset_name in GAIAWEB_DATA_SETS.keys():
        ...
        hm( f"Dataset has {dataset_num} stars.")
